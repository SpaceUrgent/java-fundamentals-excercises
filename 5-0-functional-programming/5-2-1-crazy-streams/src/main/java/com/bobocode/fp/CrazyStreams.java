...
public class CrazyStreams {
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream()
                ...
                .anyMatch(email -> email.split("@")[1].contains(emailDomain));
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
        return accounts.stream()
                ...
                .map(account -> account.getBalance())
                .findAny().orElseThrow(() -> new EntityNotFoundException(String.format("Cannot find Account by email=%s", email)));
    }
    ...
    public Map<String, Set<String>> groupFirstNamesByLastNames() {
        ...
        return accounts.stream()
                .collect(groupingBy(account -> account.getLastName(), Collectors.mapping(account -> account.getFirstName(), Collectors.toSet())));
    }
    ...
    public Map<Month, String> groupCommaSeparatedFirstNamesByBirthdayMonth() {
        return accounts.stream()
                .collect(groupingBy(account -> account.getBirthday().getMonth(),
                        Collectors.mapping(account -> account.getFirstName(), Collectors.joining(", "))));
    }
